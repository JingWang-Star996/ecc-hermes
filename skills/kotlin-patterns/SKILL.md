...
description: Idiomatic Kotlin patterns, best practices, and conventions for building robust, efficient, and maintainable Kotlin applications with coroutines, null safety, and DSL builders. Adapted for Hermes Agent with Gradle/ktlint integration.
metadata:
  origin: ECC (adapted for Hermes Agent)
  category: software-development
---

# Kotlin Development Patterns (Hermes Agent 适配版)

Idiomatic Kotlin patterns and best practices for building robust, efficient, and maintainable applications. 本技能已针对 Hermes Agent 工具链进行了适配，支持通过 terminal 执行 Gradle/ktlint、通过 read_file/patch/search_files 进行代码审查和修改。
...
- Running ktlint/detekt checks via Hermes terminal
- Navigating Kotlin projects with search_files
...
```bash
# Gradle 构建
terminal("cd /path/to/project && ./gradlew build")
terminal("cd /path/to/project && ./gradlew test")
terminal("cd /path/to/project && ./gradlew detekt")

# ktlint 代码检查
terminal("cd /path/to/project && ./gradlew ktlintCheck")

# 编译单个文件（快速验证语法）
terminal("kotlinc Main.kt -include-runtime -d Main.jar && java -jar Main.jar")
```

### 代码审查 (read_file + search_files)

```
# 查找所有 Kotlin 文件
search_files(target='files', pattern='*.kt')

# 搜索特定模式（如 !! 强制解包）
search_files(pattern='!!', file_glob='*.kt')

# 搜索 GlobalScope 使用（反模式）
search_files(pattern='GlobalScope', file_glob='*.kt')

# 查找 var 声明（可变状态）
search_files(pattern='^\\s*var\\s+', file_glob='*.kt')
```
...
```
# 使用 patch 工具进行精准修改
patch(mode='replace', path='src/main/kotlin/User.kt',
      old_string='var name: String',
      new_string='val name: String')
```

### 工作流示例

1. **代码审查流程**: search_files 定位文件 → read_file 查看内容 → 识别反模式 → patch 修复
2. **构建验证流程**: terminal 运行 `./gradlew build` → 分析错误 → patch 修复 → 重新构建
3. **重构流程**: search_files 找到所有相关引用 → read_file 理解上下文 → patch 批量修改
...
val config = serverConfig {
    ...
    database("jdbc:postgresql://localhost:***@company.com")
}
```
