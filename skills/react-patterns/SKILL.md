---
name: react-patterns
description: "React 18/19 patterns including hooks discipline, server/client component boundaries, Suspense + error boundaries, form actions, data fetching, state management decision trees, and accessibility-first composition. Adapted for Hermes Agent workflow."
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [react, hooks, state-management, performance, server-components, suspense, forms, accessibility, composition, rsc]
    related_skills: [python-patterns, golang-patterns, python-testing]
    adapted_from: ECC (Elite Coding Companion)
---
...
> 备注：本技能针对 Hermes Agent 工作流做了适配。使用 `terminal()` 执行构建/测试/lint，使用 `read_file` / `search_files` 定位和审查组件，使用 `patch` 做精准编辑，使用 `write_file` 创建新组件（自动语法检查）。

## When to Activate

- Writing or modifying React function components, custom hooks, or component trees
- Reviewing JSX/TSX files (use `search_files` to locate them, `read_file` to inspect)
...
When working on React codebases with Hermes:

- **Locate files**: use `search_files(target="files", pattern="*.tsx")` to find components; `search_files(target="content", pattern="useState|useEffect", file_glob="*.tsx")` to find hook usage.
- **Read & review**: use `read_file(path, offset, limit)` for large component files — do not cat entire files through terminal.
- **Edit**: use `patch(mode="replace")` for targeted edits; prefer small surgical patches over rewriting whole components.
- **Create new files**: use `write_file` for new components/hooks; it auto-runs syntax checks on `.tsx`.
- **Run checks**: use `terminal()` to run `tsc --noEmit`, `eslint`, `vitest`, or `next lint` after edits.
- **Long builds**: use `terminal(background=true, notify_on_complete=true)` for `next build` or large test suites.
- **Game studio context**: for web dashboards and internal tools, prefer lightweight state (Zustand/Jotai over Redux) and keep bundle size in mind — dashboards often embed in iframes or Electron shells.
...
- Run `axe` in component tests via `terminal()` (e.g., `npx vitest --run`)
...
- **React Native**: Platform-specific patterns differ enough to warrant a separate skill
