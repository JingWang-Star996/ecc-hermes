---
name: rust-testing
description: "Rust testing patterns including unit tests, integration tests, async testing, property-based testing, mocking, and coverage. Follows TDD methodology. Adapted from ECC for Hermes Agent."
version: 1.0.0
...
- Reviewing or refactoring test suites in Rust codebases
...
## Hermes Agent 集成说明 / Hermes Integration

### 触发条件 / Trigger Conditions
- 用户要求编写 Rust 单元测试、集成测试、文档测试或基准测试
- 用户要求为现有 Rust 代码增加测试覆盖
- 用户提到 TDD、proptest、mockall、rstest、criterion 等关键词
- 用户要求审查或重构 Rust 测试代码

### Hermes 工作流 / Workflow
1. 使用 `terminal` 执行 `cargo test` 查看当前测试状态
2. 使用 `terminal` 执行 `cargo llvm-cov` 检查覆盖率
3. 使用 `patch` 或 `write_file` 添加/修改测试代码
4. 遵循 RED → GREEN → REFACTOR 循环
5. 使用 `search_files` 查找现有测试模式和断言风格

### 常用命令 / Common Commands
```bash
cargo test                        # 运行所有测试
cargo test -- --nocapture         # 显示 println 输出
cargo test test_name              # 按名称过滤测试
cargo test --lib                  # 仅单元测试
cargo test --test <name>          # 仅集成测试
cargo llvm-cov --fail-under-lines 80  # 覆盖率检查
```

### 注意事项 / Notes
- 在 WSL 环境下，确保 Rust toolchain 已安装（rustup）
- 对于异步测试，需要 tokio dev-dependency
- 对于属性测试，需要 proptest dev-dependency
- 对于参数化测试，需要 rstest dev-dependency
- 对于 mock，需要 mockall dev-dependency
- 覆盖率工具需要 llvm-tools-preview 组件

### 与其他技能配合 / Related Skills
- `rust-essentials`: 基础 Rust 语法和模式
- `rust-concurrency`: 并发代码的测试策略
- `rust-async`: 异步代码的测试模式
- `software-development`: 通用开发实践
