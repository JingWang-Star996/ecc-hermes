---
name: rust-patterns
description: Idiomatic Rust patterns, ownership, error handling, traits, concurrency, and best practices for building safe, performant applications — adapted for Hermes Agent.
metadata:
  origin: ECC (adapted)
...
- Running cargo build/test/clippy/fmt via Hermes terminal
- Navigating Rust projects with read_file, search_files, patch
...
## Hermes Agent Tool Integration / Hermes 工具集成

Use the following Hermes tools when working with Rust projects:

| Task | Hermes Tool | Example |
|------|-------------|---------|
| Build / check / clippy / fmt | `terminal` | `terminal(command="cargo build")`, `terminal(command="cargo clippy -- -D warnings")` |
| Run tests | `terminal` | `terminal(command="cargo test")`, `terminal(command="cargo test -- --nocapture")` |
| Read source files | `read_file` | `read_file(path="src/main.rs")` — supports pagination via offset/limit |
| Edit / patch code | `patch` | `patch(mode='replace', path="src/lib.rs", old_string=..., new_string=...)` |
| Search symbols / usages | `search_files` | `search_files(pattern="fn process", file_glob="*.rs")` |
| Find files by name | `search_files` | `search_files(target='files', pattern="*.rs")` |
| Long-running builds / tests | `terminal(background=true, notify_on_complete=true)` | For large workspaces; poll with `process(action='poll')` |

### Typical Hermes Workflow / 典型工作流

1. **Explore**: `search_files(target='files', pattern='Cargo.toml')` → `read_file(path='Cargo.toml')`
2. **Understand**: `search_files(pattern='pub fn', file_glob='*.rs')` to map public API
3. **Edit**: `patch(mode='replace', ...)` to apply changes
4. **Verify**: `terminal(command='cargo check && cargo clippy -- -D warnings && cargo test')`
5. **Iterate**: re-read with `read_file`, re-patch, re-test

### Tips / 提示

- 使用 `terminal` 运行 `cargo clippy --fix --allow-dirty` 可自动修复部分 lint。
- 对大型项目，使用 `terminal(background=true, notify_on_complete=true)` 避免阻塞。
- `search_files` 比 shell 的 grep/find 更快，优先使用。
- `patch` 的 fuzzy matching 能容忍少量空白差异，比 sed 更稳健。
...
## Tooling Integration / 工具链集成

### Essential Commands (run via Hermes `terminal`)
